...
- DFS from the takeoff cell. At each cell, try N -> E -> W -> S in order.
...
ALTITUDE_M = 1.5            # flight altitude (positive = up)
...
class RGBReceiver:
    ...
    def __init__(self, topic: str, detector: Detector):
        ...
        self._last_submitted: float = 0.0        # timestamp of last submission
        self._submit_interval: float = 0.5       # seconds between submissions (2fps)
        self.node = Node()
    ...
    def _callback(self, msg: GzImage) -> None:
        ...
        # Throttle: only submit to YOLO at _submit_interval rate
    ...
    def submit_latest_to_detector(self) -> bool:
        """Grab latest frame and submit to YOLO. Returns False if no frame available."""
        frame = self.get_frame()
        if frame is None:
            print("[RGBReceiver] No frame available to submit")
            return False
        os.makedirs("./yolo_rgb_debug", exist_ok=True)
        
        timestamp = time.strftime("%H%M%S")
        fname = f"./yolo_rgb_debug/{timestamp}.png"
        cv2.imwrite(fname, frame)
        
        self.detector.submit_image(frame, context={"timestamp": time.time()})
        return True
...
class QualifierMission:
    def __init__(self):
        ...
        self.barrel_logger = BarrelLogger(
            ...
            on_new_barrel=self._on_new_barrel,
        )
        self.detector = Detector(
    ...
    def barrel_in_cell(self, cell):
        cn, ce = cell
        n, e = self._cell_to_ned(cell)

        for b in self.barrel_logger.get_barrels():
            bn, be = b["north"], b["east"]

            dist = math.hypot(n - bn, e - be)
            if dist < 2.0:   # safety radius
                return True

        return False
    def _ned_to_cell(self, north: float, east: float) -> Tuple[int, int]:
        cn = int(round((north - self.origin_north) / CELL_SIZE_M))
        ce = int(round((east - self.origin_east) / CELL_SIZE_M))
        return (cn, ce)
    def _on_new_barrel(self, pos_ned, confidence, class_name):
        cell = self._ned_to_cell(pos_ned[0], pos_ned[1])

        print(
            f"[Mission] ⚠️ Barrel in cell {cell} "
            f"(conf={confidence:.2f}, class={class_name})"
        )

    # optional: mark as blocked in DFS
    ...
    # ----------------------------------------------------------------------
    async def _check_blocked_ahead(self) -> bool:
        NUM_SAMPLES = 5
        SAMPLE_WINDOW_S = 1.0
        MAX_SENSOR_RANGE = 15.0
        threshold = CELL_SIZE_M + 0.5
        BLOCKED_PIXEL_RATIO = 0.30
        # Minimum clearance — catches thin obstacles like barrels
        # Set to drone body width + safety margin
        MIN_CLEARANCE_M = 0.4
        ...
        for i in range(NUM_SAMPLES):
            ...
            if depth is None:
                await asyncio.sleep(SAMPLE_WINDOW_S / NUM_SAMPLES)
                continue
            ...
            if valid.size == 0:
                readings.append((MAX_SENSOR_RANGE, 0.0, MAX_SENSOR_RANGE, depth))
                await asyncio.sleep(SAMPLE_WINDOW_S / NUM_SAMPLES)
                continue

            close_pixels = np.sum(valid < threshold)
            blocked_ratio = close_pixels / valid.size
            clearance_p80 = float(np.percentile(valid, 80))
            # 5th percentile catches thin objects — robust against single
            # noisy pixels but sensitive enough to detect a barrel
            clearance_p05 = float(np.percentile(valid, 5))

            readings.append((clearance_p80, blocked_ratio, clearance_p05, depth))
            await asyncio.sleep(SAMPLE_WINDOW_S / NUM_SAMPLES)
        ...
        clearance_blocked = sum(
            1 for (p80, _, __, ___) in readings if p80 < threshold
        )
        ratio_blocked = sum(
            1 for (_, r, __, ___) in readings if r > BLOCKED_PIXEL_RATIO
        )
        # Thin obstacle check — p05 below minimum safe clearance
        thin_blocked = sum(
            1 for (_, __, p05, ___) in readings if p05 < MIN_CLEARANCE_M
        )

        is_blocked = (
            clearance_blocked > len(readings) // 2
            or ratio_blocked > len(readings) // 2
            or thin_blocked > len(readings) // 2
        )

        self._save_depth_debug(
            readings=[(p80, d) for p80, _, __, d in readings],
            threshold=threshold,
            is_blocked=is_blocked,
        )

        if is_blocked:
            worst_p80  = min(p80 for p80, _, __, ___ in readings)
            worst_ratio = max(r   for _,  r, __, ___ in readings)
            worst_p05  = min(p05 for _,  _, p05, ___ in readings)
            print(f"[Mission] BLOCKED p80={worst_p80:.2f}m "
                  f"ratio={worst_ratio:.0%} p05={worst_p05:.2f}m")
        return is_blocked
    ...
    def _save_depth_debug(
        ...
        for i, (c, _) in enumerate(readings):
            if not np.isnan(c) and c < worst_val:
                worst_val = c
                worst_idx = i
    ...
    def _next_unvisited_direction(self) -> Optional[str]:
        for dirn in DIRECTION_ORDER:
            ...
            if (
                neighbour in self.visited 
                or neighbour in self.blocked
                or self.barrel_in_cell(neighbour)
                ):
                continue
            return dirn
        return None
        ...
        try:
            while self.running:
                ...
                if action == "explore":
                    dn, de = DELTA[dirn]
                    target_cell = (
                        self.current_cell[0] + dn,
                        self.current_cell[1] + de
                    )
                else:
                    target_cell = self.stack[-1]
                ...
                if action == "explore":
                    ...
                    if await self._check_blocked_ahead():
                ...
                ok = self.rgb_receiver.submit_latest_to_detector()
                if ok:
                    print(f"[Mission] Submitted RGB frame for cell {self.current_cell}")
                else:
                    print(f"[Mission] SKIPPED RGB (no frame) at cell {self.current_cell}")
