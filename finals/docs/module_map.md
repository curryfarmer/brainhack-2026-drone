...
| Convoy marker details (ID list? DICT_6X6_250 confirmed? or QR codes?) | PARTIALLY ANSWERED (user, 2026-06-06): markers are **QR codes, 20×20 cm**. CONFIRM: literal QR (cv2.QRCodeDetector / pyhulax onboard `recognize_qr`) vs "QR" said loosely for ArUco — and whether 20×20 applies to convoy markers, pad markers, or both. S7 SHIPPED the pluggable seam: config `marker_backend` ("aruco" default \| "qr"), both paths feed the same Sighting stream — the confirmation is now a ONE-KEY config edit, zero code. **SIM-3 (2026-06-09)** measured the sim consequence on 640 px at 1.2–2.2 m: ArUco (20 cm) decodes at ALL sentry bands; a true QR (20–40 cm) is barely *located* at 1.2 m and invisible above (decode floor ~4 px/module ≫ the ~1.7 seen) — so if the markers ARE literal QR, the sentry must fly ≪1 m or the markers be far larger (sim_sessions SIM-3) |
