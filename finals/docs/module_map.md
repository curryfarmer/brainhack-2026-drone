...
| `sim/` V2a assets (`gz_camera_bridge.py`, `px4_models/x500_mono_cam_640`, `worlds/convoy_px4.sdf`, `worlds/empty_cam.sdf`, `run_vision.sh`) | ✅ | **SIM-4** | bridge = system-py3.10 gz→TCP frame forwarder (mirrors check_detection.py); x500 + INLINED 640×480 down-cam (HFOV 99.69, topic `…/x500_mono_cam_640_0/…/camera/image`); convoy_px4 = convoy + PX4 flight-sensor systems + GPS origin, 1-camera under llvmpipe (4-cam starves the EKF); `run_vision.sh` stageA/stageB orchestration (PX4 OWNS the gz server → lockstep; spawn CLEAR of robot starts) |
...
| `configs/sitl3_vision.json` | ✅ (code) | **SIM-5** | sitl3 + gazebo frames + sentry_scan; per-drone `gazebo_video_port` 5600/5601/5602 (each drone reads its OWN onboard cam via its OWN bridge — `resolve_gazebo_video_port`; multi-drone gazebo port distinctness validated); `command_timeout_s`/`mission_budget_s` sized from the measured 3-cam RTF (`run_vision.sh probe3`). 3× flight rehearsal evidence = [`sim_sessions.md`](sim_sessions.md) SIM-5 (VM) |
