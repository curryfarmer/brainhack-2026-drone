...
| `mission/phases/navigate.py` | ✅ implemented | **S11 (NAV-5)** | open-loop transit phase (assumes AIRBORNE — never Takeoff/Land): from_config resolves goal (EXACTLY ONE of `pad_id`→arena pad center / `goal_ne_m` [N,E]) + pre-plans FROZEN tuple[Leg] via visibility_graph.plan; step() per leg = `bearing_error_to_rotate(leg.heading_deg, yaw, tol, max_step)` Rotate-to-ABSOLUTE-heading (re-zeroes yaw creep every leg, no relative delta) then Move(FORWARD, round(distance_cm)); advances leg only after the Move resolves OK; Done after final Move. Aborts (WHAT/WHICH/WHY/CHECK): last_action_ok False, yaw_deg None, budget (`total_budget_s`, strict >) exceeded, per-leg rotate cap `ceil(360/max_step)+4` not converged. 0 cm-rounding leg skipped (never a refused 0 cm Move). PlanningError (goal-in-keepout) PROPAGATES; degenerate/empty plan + unknown/typo zone key = ConfigError. Geofence deferred to guards.py/NAV-8. Tunables `zone["navigate"]`: pad_id|goal_ne_m, inflation_m(>0,def 0.5), max_leg_cm(>0,def 100), heading_tol_deg(>=0,def 5), max_step_deg(>0,def 45), total_budget_s(>0,def 120). DR-oracle integration lands ~goal. 46 tests |
