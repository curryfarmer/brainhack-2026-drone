...
| `mission/phases/land_on_pad.py` | ✅ implemented | **S11 (NAV-6)** | PURE visual-servo precision landing (drone already airborne over the pad). Sub-state machine PAD_ACQUIRE (N-of-M bounded recent-frame window over valid_marker_ids; bounded Rotate scan; acquire_timeout_s) → PAD_CENTER (shared `_servo.pixel_offset_to_move`, center_persist_frames in-deadband streak; lost → re-acquire) → PAD_DESCEND (Move(DOWN) ONLY while centred AND descend_persist_frames seen; re-gates centering after each step; lost → Move(UP) + re-acquire, max_loss_retries) → LAND_COMMIT (altitude_m ≤ commit_alt_m → Land; is_flying False → Done verified). Fallback on total_budget_s / acquire timeout / retries exhausted → blind Land + Done(UNVERIFIED_LANDING …) — NEVER a battery-dead hover (every non-terminal path passes the budget gate). last_action_ok False / non-finite altitude → actionable Abort. Two valid pads → largest-bbox, lowest-id tie-break (deterministic). All tunables in `zone["land_on_pad"]` via `from_config` (search.py `_zone_kwargs` template; k_lateral/commit_alt_m are ONSITE-CALIBRATED config, gate F). Landing SLOT is NAV-8/orchestrator, not the phase. Sources: `_servo` (NAV-3 locked signs), search.py SentryScan. 63 tests + 6/6 mutation kill-check |
