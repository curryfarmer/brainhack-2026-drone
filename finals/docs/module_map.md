...
| `config.py` | ✅ implemented | S1 / **S11 (NAV-0)** | qualifier_run.py:72–132 MissionConfig; known-issue #9 weights guards. **NAV-0: optional top-level `arena_name` -> loads finals/configs/arenas/<name>.json into `FinalsConfig.arena` (ArenaMap); `arena` is DERIVED, never JSON; loud-on-missing/malformed; NAV-2 hardens the semantic checks** |
...
| `mission/planning/types.py` | ✅ contracts | **S11 (NAV-0)** | frozen map/geometry shapes (Leg, KeepOut, LandingPad, ArenaMap + thin from_dict); frame = (north_m,east_m), heading CCW+ (dead_reckon sign). NAV-2 hardens from_dict |
| `mission/planning/polygon_tools.py` | ⬜ stub | **S11 (NAV-1)** | inflate_polygon / segment_intersects_polygon / point_in_polygon (pure 2-D geometry) |
| `mission/planning/visibility_graph.py` | ⬜ stub | **S11 (NAV-1)** | `plan(start,goal,arena,inflation_m,max_leg_cm)->list[Leg]`; visibility-graph A* over inflated keep-outs; raises PlanningError (NAV-1 adds it to errors.py) |
| `mission/phases/_servo.py` | ⬜ stub | **S11 (NAV-3)** | shared PURE servo math: wrap180/clamp/bearing_error_to_rotate/pixel_offset_to_move; sign matches perception.py (yaw MINUS px offset). DRY home for land_on_pad + track_convoy after s11 merges |
