...
| S-NAV   | ☑ | ☑ | 0 | 0 | FIXED: rotate-cap Abort residual now WRAPPED (`wrap180(target-yaw)`, was unwrapped — printed e.g. 350 deg for a true -10 deg error) + residual-VALUE test (heading 180 / yaw -170 asserts ~-10, NOT 350 — the wrap fix was previously unkillable); rotate-cap `>` boundary test (kills `>`→`>=`, asserts the Abort fires after exactly rot_cap Rotates); :322 recursion → bounded while loop (budget/last_action_ok/yaw guards kept ABOVE the loop; behavior identical). ACCEPTED: :322-recursion was bounded+harmless (now refactored anyway, no behavior change); the :254 final-leg-over-budget + :266 mid-plan-yaw-None LOW TEST gaps are belt-and-suspenders (S4 agent `_fail()`s before re-stepping) — left. | 1017 ✓ |
| S-LAND  | ☑ | ☑ | 0 | 0 | FIXED (tests, 4 surviving mutants killed): commit-at-EXACTLY-commit_alt_m → Land (kills `<=`→`<`); descend gate at EXACTLY descend_persist_frames → DOWN + one-below → Hold (kills `>=`→`>`); budget-in-DESCEND → Fallback Land (regression guard for the centralized budget gate); decoy-in-CENTER (decoy-only→lost) + decoy+valid-in-frame (centres on the VALID id only) (kills the valid-id-filter-drop). ACCEPTED (document-only, no descent-LOGIC change before SITL): descend confirm-window shares center-hold frames toward the first descend confirm (MED, soft/bounded/not-a-hover-risk — code comment added at the gate site); `_loss_retries` cumulative not consecutive (LOW, conservative-safe); is_flying None-after-Land never reaches Done (LOW, no hover); `_pick_target` per-tick lambda (LOW OPT, ~nil). | 1017 ✓ |
| S-DECON | ☑ | ☑ | 0 | 0 | FIXED: config.py cross-drone landing-pad target guard (`_validate_pad_targets`) — refuses (a) duplicate navigate.pad_id across drones, (b) a navigate pad_id whose arena pad is RED (valid=false), (c) fewer arena VALID pads than navigating drones; goal_ne_m coord-goals exempt from (a)/(b), counted in (c). test_landing_config dup-pad + red-pad tests REWRITTEN to assert ConfigError refusal (were observability-only) + a (c) capacity test + a shipped-config-loads-clean regression. Mutant: delete the guard → all 3 refusal tests FAIL → restored. ACCEPTED (S-DECON owns, not in this NAV file scope / advisory-by-design): SectorGuard lazy import (LOW OPT, no cycle), guards.py `Tuple` annotation not imported (LOW CONV), identical/swapped sector_deg (LOW, sectors ADVISORY — never hard-refuse), valid_marker_ids placeholders (defer-to-gate-F). | 1017 ✓ |
| E2E suite | ☐ | — | - | - | | |
...
**Batch-2 mutation kill-checks (verified, all reverted clean):** delete `_validate_pad_targets(cfg)` → dup-pad + red-pad + too-few-valid-pads tests FAIL (config.py); unwrapped residual (`leg.heading_deg - yaw`) → residual-value test FAILS (prints 350, expects -10); rotate-cap `>`→`>=` → boundary test FAILS (Aborts after 11 Rotates, expects 12); commit `<=`→`<` → commit-at-exactly test FAILS; descend gate `>=`→`>` → descend-at-exactly test FAILS; drop the `marker_id in valid_marker_ids` filter → both decoy-in-CENTER tests FAIL; budget gate bypassed in DESCEND → budget-in-DESCEND test FAILS. (Backed up each file to /tmp, applied mutant, ran the targeted test, restored from backup — avoids the batch-1 `git checkout` footgun.)
