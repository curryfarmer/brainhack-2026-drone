...
#: PerceptionLoop + agent frame-ts). DELIBERATELY narrower than
#: `frame_backend != "none"` — see the VISION GATE wiring note in the module
#: docstring (a backend declared in a config but not yet wired would crash on
#: its stub or false-DEGRADE).
...
def _build_perception(cfg: FinalsConfig, drone_id: str, bus: SightingBus,
    ...
    _frames_wired(cfg). Today that is ReplaySource OR — when frame_backend is
    "pyhulax" (bench/real, S10) — a PyhulaxVideoSource over the per-drone shared
    DroneAPI passed in `api` (the SAME link the flight adapter uses). The source
    is NOT started here: preflight P6 owns start() AFTER P4 connects the api
    (the connect-before-stream-start ordering). S8 adds a gazebo branch here."""
